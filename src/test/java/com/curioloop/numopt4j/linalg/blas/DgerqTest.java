...
class DgerqTest {
    ...
    @Nested
    @DisplayName("DGERQF Blocked vs Unblocked")
    class DgerqfComparisonTests {

        @Test
        @DisplayName("Square matrix")
        void testSquareMatrix() {
            testDgerqfBlockedVsUnblocked(50, 50);
        }

        @Test
        @DisplayName("Wide matrix")
        void testWideMatrix() {
            testDgerqfBlockedVsUnblocked(30, 100);
        }

        @Test
        @DisplayName("Tall matrix")
        void testTallMatrix() {
            testDgerqfBlockedVsUnblocked(100, 30);
        }

        @Test
        @DisplayName("Large matrix")
        void testLargeMatrix() {
            testDgerqfBlockedVsUnblocked(200, 200);
        }

        private void testDgerqfBlockedVsUnblocked(int m, int n) {
            ...
            Dgerq.dgerqf(m, n, aBlocked, 0, lda, tauBlocked, 0, workBlocked, 0, lworkBlocked);
            Dgerq.dgerq2(m, n, aUnblocked, 0, lda, tauUnblocked, 0, workUnblocked, 0);
    ...
    @Nested
    @DisplayName("DGERQF Correctness")
    class DgerqfCorrectnessTests {

        @Test
        @DisplayName("RQ decomposition: A = R * Q")
        void testRQDecomposition() {
            testRQDecomposition(50, 50);
            testRQDecomposition(30, 100);
            testRQDecomposition(100, 30);
        }

        private void testRQDecomposition(int m, int n) {
            Random rnd = new Random(12345);
            int k = min(m, n);
            int lda = n;
            ...
            Dgerq.dgerqf(m, n, a, 0, lda, tau, 0, work, 0, lwork);

            double[] r = extractR(a, m, n);
            double[] q = generateQFromRq(a, m, n, k, tau);

            double[] aReconstructed = matMul(r, m, n, q, n, n);

            assertMatrixClose(original, aReconstructed, m, n, TOL,
                String.format("A != R*Q for m=%d, n=%d", m, n));
        }
        ...
        private void testQOrthogonality(int m, int n) {
            ...
            assertMatrixClose(identity, qqt, n, n, TOL * max(1, n),
                String.format("Q*Q^T != I for m=%d, n=%d", m, n));
        }
    }

    @Nested
    @DisplayName("DORGRQ Blocked vs Unblocked")
    class DorgrqComparisonTests {

        @Test
        @DisplayName("Square matrix")
        void testSquareMatrix() {
            testDorgrqBlockedVsUnblocked(50, 50);
        }

        @Test
        @DisplayName("Wide matrix")
        void testWideMatrix() {
            testDorgrqBlockedVsUnblocked(30, 100);
        }

        private void testDorgrqBlockedVsUnblocked(int m, int n) {
            Random rnd = new Random(12345);
            int k = min(m, n);
            int lda = n;
            ...
            int nbRq = Ilaenv.ilaenv(1, "DGERQF", " ", m, n, -1, -1);
            int lworkRq = m * nbRq + nbRq * nbRq;
            double[] workRq = new double[lworkRq];
            Dgerq.dgerqf(m, n, original, 0, lda, tau, 0, workRq, 0, lworkRq);
            ...
            Dgerq.dorgrq(m, n, k, qBlocked, 0, lda, tau, 0, workBlocked, 0, lworkBlocked);
            Dgerq.dorgr2(m, n, k, qUnblocked, 0, lda, tau, 0, workUnblocked, 0);
    ...
    @Nested
    @DisplayName("DORGRQ Correctness")
    class DorgrqCorrectnessTests {

        @Test
        @DisplayName("Q orthogonality")
        void testQOrthogonality() {
            testDorgrqOrthogonality(50, 50);
            testDorgrqOrthogonality(30, 100);
        }

        private void testDorgrqOrthogonality(int m, int n) {
            Random rnd = new Random(12345);
            int k = min(m, n);
            int lda = n;
            ...
            int lworkRq = m * nbRq + nbRq * nbRq;
            double[] workRq = new double[lworkRq];
            Dgerq.dgerqf(m, n, a, 0, lda, tau, 0, workRq, 0, lworkRq);

            int nb = Ilaenv.ilaenv(1, "DORGRQ", " ", m, n, k, -1);
            int lwork = m * nb + nb * nb;
            double[] work = new double[lwork];

            Dgerq.dorgrq(m, n, k, a, 0, lda, tau, 0, work, 0, lwork);
    ...
    private static double[] extractR(double[] a, int m, int n) {
        int k = min(m, n);
        double[] r = new double[m * n];
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                int colStart = n - k;
                if (j >= colStart && j - colStart >= k - m + i) {
                    r[i * n + j] = a[i * n + j];
                } else {
                    r[i * n + j] = 0;
                }
    ...
    private static double[] generateQFromRq(double[] a, int m, int n, int k, double[] tau) {
