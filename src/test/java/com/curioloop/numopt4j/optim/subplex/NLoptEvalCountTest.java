...
public class NLoptEvalCountTest {
    ...
    private static void test(String name, ToDoubleFunction<double[]> f,
    ...
    public static void main(String[] args) {
        ...
        // 1D
        test("1D Oscillating", x -> { double y=x[0]-1.23456789; return sqr(y*0.1)-Math.cos(y-2*Math.sin(3*y)); },
                new double[]{-5}, new double[]{5}, 2000, 47, -0.765342);

        // 2D
        test("Rosenbrock", x -> 100*sqr(x[1]-x[0]*x[0])+sqr(1-x[0]),
                new double[]{-2,-2}, new double[]{2,2}, 5000, 175, 1.07607e-7);

        test("McCormic", x -> Math.sin(x[0]+x[1])+sqr(x[0]-x[1])-1.5*x[0]+2.5*x[1]+1,
                new double[]{-1.5,-3}, new double[]{4,4}, 5000, 73, 1.22837);

        test("Goldstein-Price", x -> {
        ...
        test("Six-hump Camel", x -> 4*sqr(x[0])-2.1*Math.pow(x[0],4)+Math.pow(x[0],6)/3.0+x[0]*x[1]-4*sqr(x[1])+4*Math.pow(x[1],4),
                new double[]{-5,-5}, new double[]{5,5}, 5000, 99, -1.03163);

        test("Branin", x -> { double a=1-2*x[1]+0.05*Math.sin(PI4*x[1])-x[0],b=x[1]-0.5*Math.sin(PI2*x[0]); return sqr(a)+sqr(b); },
                new double[]{-10,-10}, new double[]{10,10}, 5000, 81, 0.102164);

        test("Shubert", x -> { double f=0; for(int j=1;j<=5;j++) for(int i=0;i<2;i++) f-=j*Math.sin((j+1)*x[i]+j); return f; },
                new double[]{-10,-10}, new double[]{10,10}, 10000, 95, -5.2611);

        test("Hansen", x -> { double a=0,b=0; for(int i=1;i<=5;i++) a+=i*Math.cos((i-1)*x[0]+i); for(int i=1;i<=5;i++) b+=i*Math.cos((i+1)*x[1]+i); return a*b; },
                new double[]{-10,-10}, new double[]{10,10}, 10000, 136, -176.542);

        // 3D
        test("Box-Betts", x -> { double f=0; for(int i=1;i<=10;i++){double e0=Math.exp(-0.1*i*x[0]),e1=Math.exp(-0.1*i*x[1]),e2=Math.exp(-0.1*i)-Math.exp(-1.0*i);f+=sqr(e0-e1-e2*x[2]);}return f; },
        ...
        test("Corner 4D", x -> { double u=x[0]+x[1]*x[2]*Math.sin(2*x[3]),v=x[0]+2*Math.sin(u); return 1+v*v+0.1*(x[1]+x[2]+x[3]); },
        ...
        test("Paviani", x -> { double f=0,p=1; for(int i=0;i<10;i++){f+=sqr(Math.log(x[i]-2))+sqr(Math.log(10-x[i]));p*=x[i];}return f-Math.pow(p,0.2); },
                p10l, p10u, 100000, 540, -45.7785);

        double[] g10l=new double[10],g10u=new double[10]; for(int i=0;i<10;i++){g10l[i]=-500;g10u[i]=600;}
        test("Griewank", x -> { double f=1,p=1; for(int i=0;i<x.length;i++){f+=sqr(x[i])*0.00025;p*=Math.cos(x[i]/Math.sqrt(i+1.0));}return f-p; },
                g10l, g10u, 100000, 605, 0.0344921);

        test("Convex Cosh", x -> { double f=1; for(int i=0;i<x.length;i++) f*=Math.cosh((x[i]-i)*(i+1)); return f; },
        ...
        test("Gen.Rosenbrock 30D", x -> { double f=0; for(int i=0;i<29;i++) f+=100*sqr(x[i+1]-x[i]*x[i])+sqr(1-x[i]); return f; },
    ...
    private static ToDoubleFunction<double[]> shekelFunc(int m) {
        ...
        return x -> { double f=0; for(int i=0;i<m;i++) f-=1.0/(c[i]+sqr(x[0]-A[i][0])+sqr(x[1]-A[i][1])+sqr(x[2]-A[i][2])+sqr(x[3]-A[i][3])); return f; };
    }

    private static ToDoubleFunction<double[]> levyFunc() {
        return x -> { int n=x.length; double a=x[n-1]-1,b=1+sqr(Math.sin(PI2*x[n-1])); double f=sqr(Math.sin(PI3*x[0]))+a*b;
            for(int i=0;i<n-1;i++){double ai=x[i]-1,bi=1+sqr(Math.sin(PI3*x[i+1]));f+=sqr(ai)*bi;} return f; };
    }
}
